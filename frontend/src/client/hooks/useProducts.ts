...
import { fetchAiProductComparison } from '../services/productComparisonService'
import type {
  AiProductComparison,
  ListProductRes,
  Product,
...
export const useCompareProducts = (ids: number[], enabled: boolean = true) =>
  useQuery<Product[]>(
    {
      queryKey: ['compareProducts', ids],
...
export const useAiProductComparison = (ids: number[], enabled: boolean = true) =>
  useQuery<AiProductComparison>({
    queryKey: ['aiProductComparison', ids],
    queryFn: () => fetchAiProductComparison(ids),
