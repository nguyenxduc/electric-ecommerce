...
import type { ProductSpecDetail } from '../../types/product'
import { flattenProductSpecs } from '../../utils/productSpecs'
...
const TechnicalDetail = ({ specs }: TechnicalDetailProps) => {
  ...
  const flattenedSpecs = flattenProductSpecs([], specs)
  const visibleCount = expanded ? flattenedSpecs.length : 5
  return (
    <div className="w-full min-w-[500px] rounded-2xl bg-white p-5 sm:p-6 border-gray-100">
      ...
      <dl className="divide-y divide-gray-100">
        {flattenedSpecs.slice(0, visibleCount).map((item, i) => (
          <div
            key={i}
